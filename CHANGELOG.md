...
## 修复：删掉媒体库里的目录后，目录同步又把它整份拷了回来

「媒体整理 → 历史」里删掉媒体库文件（只删目的端、保留源文件）之后，下一次目录同步
会把这个文件当成从没整理过的新文件，重新识别、重建目录、**把刚删掉的那一份又拷回媒体库** ——
删了又回来，反复出现。

### 现象与原因

去重标记被「删文件」这个动作顺手清掉了。识别重命名模式下，判断「这个文件整理过没有」
看的是 `TRANSFER_BLACKLIST` 里的一条记录（键为**源文件路径**）。而 `delete_history`
（历史页的删除按钮）在判断删除范围**之前**就无条件执行了
`delete_transfer_blacklist(源路径)` —— 三个口径（只删源文件 / 只删媒体库文件 / 两者都删）
全都会执行。

于是「只删媒体库文件」时：源文件还在原处，但它的「已整理过」标记已经没了 →
下一轮扫描看起来就是个新文件 → 目的目录不存在 → 重建 + 整份拷回。

另外两处也会让标记消失，一并修掉：

- **「清空记录」连带清掉整张去重表**：原写法 `TRANSFERHISTORY.delete() and
  TRANSFERBLACKLIST.delete()`，清空看板记录的同时把去重状态一起重置了
  （历史为空时还会因 `and` 短路，行为自相矛盾）。清空记录不该等于让媒体库被重新整理一遍。
- **「目的文件已存在」的分支不写标记**：这类文件永远入不了账，每一轮全量扫描都会
  重新识别一遍、重复告警，**不会自愈**。
...
| 位置 | 改动 |
|---|---|
| `web/action.py` `delete_history` | 删标记挪进 `del_source` / `del_all` 分支：源文件都不在了，标记没有意义；**「只删媒体库文件」保留标记** |
| `db_helper.py` `truncate_transfer_history_list` | 只清 `TRANSFER_HISTORY`，「清空记录」不再重置去重状态 |
| `db_helper.py` `delete_transfer_blacklist` | 删除前也做 `os.path.normpath` |
| `filetransfer.py` `transfer_media` | 目的已存在**且大小一致**时补记标记 |

**「清理转移缓存」（`/tbl`、服务页按钮）仍然整表清空这两张表，行为不变** ——
想让整个媒体库强制重新整理一遍，还是用它。这是唯一的重置入口。

**「更新过」怎么算**：目的已存在但**大小不一致**时不记标记，保留洗版机会
（新文件更大时照旧覆盖）；大小一致才认定「就是这份文件，已经整理过了」。

**顺手修掉一个 Windows 上的隐藏 bug**：`delete_transfer_blacklist` 用 `str(path)` 做比较，
而写入时存的是 `os.path.normpath(path)`（Windows 上会把 `/` 换成 `\`）→ **按路径删除永远匹配不到**，
表现为「删了但没删掉」。Linux 上两种写法恰好一致，所以长期没暴露。现在写入 / 查询 / 删除
统一走同一个归一化。

### 语义变化（需要知道）

「删除媒体库文件」不再是「单条重新整理」的入口 —— 它现在只删文件、不动标记。
要单条重做请用**「重新识别」**：它走手动整理（`SyncType.MAN`），而去重标记只在
目录同步（`MON`）路径上生效，**本来就不受标记约束**。

升级**不需要数据迁移**：现有 `TRANSFER_BLACKLIST` 记录原样生效，
**不会**因为升级触发一次全量重拷。

# v6.0.3 (2026-09-23)

## 新功能：刷流任务可设「任务时长」，到点自动停止

刷流任务以前只能手动启停 —— 想跑 2 小时就得设个闹钟回来点「停用」，不然它会一直跑。
现在任务级新增一个「**任务时长**」，填小时数即可。

- **单位小时、支持小数**：`0.5` = 30 分钟、`2` = 2 小时、`24` = 一天；**留空 = 不限时**（与以前一样）。
- **取值范围 0.1 ~ 720 小时**（6 分钟 ~ 30 天）。前后端共用同一个归一函数，开放 API 也走同一道校验；
  填了非法值会被直接拦下并提示，**不会静默按「不限时」处理**。
- **到点自动停止**：把状态置为「停用」，并清空计时起点。**已在跑的种子不动** ——
  已有种子的收尾交给既有的删种规则，不做强删。
- **任务卡片新增「剩余时间」**：按秒递减，倒计时在前端本地跑、不轮询后端；
  后端只在页面装载时给一个初值。

计时起点独立于「创建时间」—— 因为「停用」后再「启动」必须**重新计时**。规则是：
状态变为「正常」时写入当前时间，变为「停用 / 暂停」时清空。所以同一任务反复启停，
每次启动都是完整的「任务时长」。

检查粒度是 **60 秒**：这里单独挂了一个短周期任务，**没有**复用 300 秒的删种扫描 ——
否则 6 分钟（0.1 小时）的任务最迟要等 5 分钟才停。

## 改进：移除刷流任务里两处速度限制入口，统一交给下载器

刷流任务里原本有两对速度字段，职责重叠。这次全部移除：

| 位置 | 字段 | 实际情况 |
|---|---|---|
| 任务配置 | 上限上传速度 / 上限下载速度 | **已失效的死开关** —— 判定分支只写日志、不拦截，改什么值都不会生效 |
| 选种规则区 | 上传限速 / 下载限速（KB/S） | 真正会写进下载器的限速 |

两对一起清掉，**速度限制统一由下载器自身设置**，避免「两个地方各配一半、互相覆盖」。
清理覆盖前端表单与校验、后端取值、开放 API 参数、数据模型列、DB 读写。
老任务里遗留的 `upspeed` / `downspeed` 读出来只是没人引用，**不需要数据迁移**。

⚠️ 删种规则里的「平均上传速度」（`avg_upspeed`）是另一套东西（判断种子跑够没有），**未受影响**。

## 修复：刷流任务弹窗残留的校验红框

校验不通过时字段会标红；但关闭弹窗再打开（新建或编辑），**红框不会消失** ——
上一次的红色提示会挂在一个内容全新的表单上。

现在两个入口（新建 / 编辑）都在弹出前先清掉弹窗内的 `.is-invalid`。
清理**只动样式类**：不清输入值、不动焦点，所以不会误伤任何已填内容。

## 改进：「下载目录设置」字体统一 + 折叠箭头更醒目

「下载器设置」弹窗里，「下载目录设置」这一块原先自成一套字号（标题 15px 深色、
卡片标签 12px 灰），与同页面「监控 / 转移方式 / 标签隔离 / 目录隔离」的表单标签不是一套。

- **字体令牌集中定义**：字号 / 字重 / 颜色 / 行高 / 字间距在 `#modal-downloader` 上定义一次，
  其余规则只引用令牌 —— 以后调整只改一处，各块之间不会再各自漂移。
- **折叠标题改为与分区标题同源**（13px 灰），卡片标签直接挂 Tabler 的 `.form-label`；
  紧凑控件（`-sm`）只保留更小的内边距，文本字号回到与 `.form-select` / `.form-control` 一致。
- **折叠箭头更醒目**：原来用 `▸`（U+25B8，Unicode 里名字就叫「**小**三角」），
  同字号下明显更小更细，换成 `▶`（U+25B6 实心三角）；字号改为**跟随折叠标题**
  （符号与标题永远一样大），标题字号以后再调，箭头自动跟随。

# v6.0.2 (2026-09-23)

## 改进：「分类标签」与「自动分类」两列合并成一个下拉框

下载目录设置里原本有两个都跟「分类」有关的列：「分类标签」填 qB 分类名，
「自动分类」选「自动 / 电影 / 电视剧 / 其它」。两列的职责其实不同（一个给名字、一个决定
要不要按类型自动扫），但并排放在同一行里，看起来像重复配置 —— 配一行要在两列之间来回看，
很容易只填了一列就以为配好了。

### 改动

合并成**一列「分类标签」**，控件是一个下拉 + 一个输入框：

| 下拉 | 输入框 | 含义 |
|---|---|---|
| 自动判定 | （隐藏） | 参与自动分类扫描，分类名由系统按媒体类型推导（动漫并入电视剧） |
...
存储**复用原 `auto_category` 字段**，原 `label` 字段置空、不再写入：
...
- `auto_category` 为空、`label` 有值 ⇒ 当作**自定义**（行为与升级前完全一致，**不参与扫描**）；
- `auto_category` 是旧的「固定档」值（电影 / 电视剧 / 其它）⇒ 同样当作自定义分类名，不参与扫描。

**老数据只在读取时归一、不回写数据库** —— 升级后打开看一眼不保存，不会改动你的配置。

### 一个要记住的保留值

「**自动**」这两个字是「自动判定」的保留值，所以选「自定义」时**不能把分类名填成「自动」**。
提交时会直接拦下并给出提示 —— 不拦的话它会被后端读成自动判定，
等于在你不知情的情况下开启了扫描。

### 顺带

- 下载目录行的**规则条数徽标改为只统计「自动判定」的行**（以前把两列都算进去，数字偏大）；
- `pt` 段配置注释同步（主配置只是兜底，真正的入口在「下载器设置」那张表里）。
