...
### ✨ 新功能
- 目录清理：服务菜单新增「目录清理」，批量删除指定根目录下、总大小不超过阈值的超小子目录（清理失败任务 / 样本残留）。支持「预览」（仅统计、不真删）与「执行」两步；亦可通过 REST 接口 `/system/clean_dirs/scan`（预览）与 `/system/clean_dirs/run`（执行）调用。
...
**完整更新记录**: [v6.0.6...v6.0.7](https://github.com/carl800-1/nas-tools/compare/v6.0.6...v6.0.7)
...
### 🐛 修复
- 刷流弹窗版式收紧：保存目录上移与「标签 / 任务时长」同排并缩短为三分之一宽；四个开关间距压缩，确保窄屏下一排显示（修复 v6.0.5 真实浏览器中开关折行的问题）。

### 🔧 其他
- 无

**完整更新记录**: [v6.0.5...v6.0.6](https://github.com/carl800-1/nas-tools/compare/v6.0.5...v6.0.6)
...
### ✨ 新功能
- 刷流任务弹窗精简：移除 RSS 地址输入框（改用站点级配置），将消息推送 / 转移到媒体库 / 限免限速 / 限免删种四个开关排成一行。
- 选种规则精简：移除「同时下载任务数」「部分下载（拆包）规则」，将包含 / 排除条件移至选种规则末尾。

### 🐛 修复
- 修复刷流「转移到媒体库」开关失效：`__link` 两个入口补守卫，不勾「识别重命名」时开关不再误生效；种子刚完成的一瞬增加 5 秒兜底重查，消除 60 秒缓存窗口内的漏判。

### 🔧 其他
- 无

**完整更新记录**: [v6.0.4...v6.0.5](https://github.com/carl800-1/nas-tools/compare/v6.0.4...v6.0.5)
