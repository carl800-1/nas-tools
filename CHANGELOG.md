> 各版本的详细改动（改了哪些文件、测试、为什么这么改）见 [CHANGELOG_DETAIL.md](CHANGELOG_DETAIL.md)。

# v6.0.7 (2026-09-24)

## ✨ 新增：目录清理
批量删除指定根目录下、总大小不超过阈值的子文件夹，专门清理下载目录里残留的失败任务 / 样本小目录。

### 怎么用
1. 打开 **服务 → 目录清理**
2. 填 **根目录**（要清理的目录）和 **阈值(MB)**（子文件夹总大小 ≤ 该值才删）
3. 点 **预览**：先看会删哪些、预计释放多少空间（只统计、不真删）
4. 确认后点 **执行**，二次确认即开始删除

> 阈值留空或非法会自动按 0 处理，避免误删有内容的目录；也提供 REST 接口 `/system/clean_dirs/scan`（预览）和 `/system/clean_dirs/run`（执行）。

---

# v6.0.6 (2026-09-24)

## 🐛 修复：刷流弹窗版式（4 个开关一排、保存目录缩短）
v6.0.5 在真实浏览器里 4 个开关仍折行、保存目录独占整行偏浪费。

### 改动
- 保存目录上移与「标签 / 任务时长」同行，缩短为三分之一宽
- 4 个开关间距压缩，确保一排放得下
...
## ✨ 改进：刷流任务弹窗精简（只留真正要配的项）
去掉 RSS 地址输入框、同时下载任务数、部分下载（拆包）规则；包含 / 排除移到选种规则末尾；4 个开关排一排。

## 🐛 修复：刷流「转移到媒体库」开关的两处缺口
- 不勾「识别重命名」时开关也会生效（`__link` 两个入口补守卫）
- 种子刚下载完成的一瞬不会被误整理（5 秒兜底重查）

> ⚠️ 未重新保存的老任务仍按旧值跑（卡片继续显示「同时下载: N」徽标）。
