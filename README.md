...
| 类别 | 覆盖 |
|---|---|
| 查询（17 个） | 下载中 / 已下载、转移历史与统计、站点列表与流量数据、电影与剧集订阅、订阅历史、下载器、索引器、媒体信息、媒体库空间、未识别文件、站点最新资源、系统进程、版本检查 |
| 操作（9 个） | 添加 / 删除订阅、搜索并下载、启停下载任务、删除下载、重新识别未识别文件、刷新订阅、执行刷流任务、执行目录同步 |
| 运维（4 个） | 重启服务、系统升级、执行删种、清空历史记录 |
...
|---|---|
| `在下载什么？` | 查询下载器实时任务与进度 |
| `我的站点数据怎么样？` | 返回各站上传量、分享率、做种数 |
| `订阅了哪些剧？` | 列出剧集订阅与已下载进度 |
| `帮我订阅沙丘 2021` | 先核对影片信息，再添加订阅 |
| `把漫长的季节重新搜一下` | 触发该订阅重新搜索资源 |
| `磁盘还剩多少？` | 返回媒体库空间占用 |
| `重启一下服务` | **先让你确认**，回复「确认」才执行 |

**危险操作二次确认**：删除、重启、升级、清空历史等操作，机器人会先给出确认提示，**只有回复「确认」才执行**，回复其它内容即取消；确认请求 5 分钟内有效。
...
> **本地模型**：本项目走标准 OpenAI 接口协议，配合 OMLX / Ollama / vLLM 等本地推理服务即可离线运行 AI 助手——`api_url` 指向服务地址、`model` 填模型名即可。建议选用支持 function calling 的模型（如 Qwen3 系列），体验最接近云端。
