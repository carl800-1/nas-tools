    ...
    def sync_mediaserver(self):
        ...
        with lock:
            ...
            if not all_libraries:
                ...
                return
            # 根据同步范围配置过滤要同步的库
            # sync_library 可能包含: "movie", "tv", "" (全部), 或具体的库 ID
            libraries_to_sync = []
            for library in all_libraries:
                lib_id = str(library.get("id", ""))
                lib_type = library.get("type", "")
                if not sync_library or "all" in sync_library or "" in sync_library:
                    # 同步全部
                    libraries_to_sync.append(library)
                elif "movie" in sync_library and lib_type in [MediaType.MOVIE.value, "movie", "Movie"]:
                    libraries_to_sync.append(library)
                elif "tv" in sync_library and lib_type in [MediaType.TV.value, "tv", "TV", "Series", "show"]:
                    libraries_to_sync.append(library)
                elif lib_id in [str(x) for x in sync_library]:
                    # 按具体库 ID 同步
                    libraries_to_sync.append(library)
            if not libraries_to_sync:
                log.info("【MediaServer】没有需要同步的媒体库")
